...
class Metric:
    ...
    def eval_rating(self, idx, test_ratings, test_negatives, K, model):
        ...
        """
        items = test_negatives[idx]      # negative items [neg_itemId, ... ] (1,100)
        user_idx = test_ratings[idx][0]  # [user_id, itemId][0]
        holdout = test_ratings[idx][1]   # [user_id, itemId][1]
        items.append(holdout)            # holdout 추가 [neg_itemId, ..., holdout] (1,101)

        # prediction
        predict_user = np.full(len(items), user_idx, dtype='int32').reshape(-1, 1)  # [[user_id], ...], (101, 1)
        np_items = np.array(items).reshape(-1, 1)                                   # [[itemId], ... ], (101, 1)
    ...
    def evaluate_top_k(self, df_neg, df_test, model, K=10):
        ...
        test_i = df_test['itemId'].values.tolist()

        test_ratings = list(zip(test_u, test_i))
        df_neg = df_neg.drop(df_neg.columns[0], axis=1)
        test_negatives = df_neg.values.tolist()  # [[(userId, item_id=holdout)], neg_item,... ] (1,100)
